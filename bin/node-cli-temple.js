...
program.name(binName).version(pkgInfo.version);

program.option('-v, --verbose', '[-|+] see verbose', false).on('option:verbose', function () {
  nlog.open_verbose();
});
program.option('--log', '[-|+] open log file out put', false).on('option:log', function () {
  nlog.file(`logs/${binName()}-${moment(new Date(), moment.defaultFormat).format('YYYY-MM-DD-HH-mm')}.log`);
});
program.option('-f, --force', '[-|+] force mode', false).on('option:force', function () {
  config.open_force();
  nlog.warning('! now in force mode');
});
program.option('--no-color', '[+|-] close color cli out put', false).on('option:no-color', function () {
